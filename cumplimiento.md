...
| 1.3 | PoC: firmado digital cualificado de documentos XML/PDF + verificación | ✅ Cumplido | `Consulta_1/2_firma_verificacion_poc.py` — implementa PKCS#7/CMS detached signature, CLI con subcomandos `sign`/`verify`, validación contra bundle CA; verificado en `Evidencias_Final/Consulta_1/informe_demo.xml.p7s` |
| 1.4 | El informe debe ser firmado digitalmente por los consultores que lo entregan | ⚠️ Parcial | La PoC de firma está verificada, pero no existe evidencia de PDF final firmado con DNIe/Autofirma o QSCD por los consultores |
...
| 2.6 | Contexto: estado del dispositivo (postura de seguridad: AV, firewall, OS, parches) | ✅ Cumplido | `access_policy.json` campo `required_posture`; broker evalúa `av_active`, `firewall_active`, `disk_encrypted`, `os_patched` |
| 2.7 | Contexto: ubicación, horario y condiciones (hospital, horario laboral, cita con paciente) | ✅ Cumplido | `access_policy.json` campos `allowed_locations`, `allowed_networks`, `working_hours`, `require_appointment_match`; soporte timezone Europa/Madrid |
...
| # | Objetivo / Requisito | Estado | Evidencia |
|---|---|---|---|
| 3.1 | Implementar subasta Vickrey inversa (gana la puja más baja, paga el segundo precio más bajo) | ✅ Cumplido | `Consulta_3/contracts/HealthProcurementAuction.sol` — lógica de ganador (bid más bajo) y precio pagado (segundo precio distinto) |
| 3.2 | Pujas deben ser entero positivo mayor que cero | ✅ Cumplido | `contracts/HealthProcurementAuction.sol` — validación en fase reveal: `require(_amount > 0)` |
| 3.3 | Valor máximo de puja definido por Servicio de Salud | ✅ Cumplido | `contracts/HealthProcurementAuction.sol` — parámetro `maxAcceptablePrice` en constructor; pujas superiores rechazadas |
| 3.4 | Un mismo pujador no puede pujar dos veces | ✅ Cumplido | `contracts/HealthProcurementAuction.sol` — mapping de commits, `require(!bidders[msg.sender].committed)` |
| 3.5 | Subasta cierra con 30 pujas o al llegar al deadline | ✅ Cumplido | `contracts/HealthProcurementAuction.sol` — constante `MAX_BIDS = 30`, cierre por count o por `block.timestamp > biddingDeadline` |
| 3.6 | Confidencialidad de pujas hasta cierre (nadie sabe la puja de otro antes de tiempo) | ✅ Cumplido | `contracts/HealthProcurementAuction.sol` — esquema commit-reveal: commit es hash(bid+salt), valor real solo en fase reveal |
| 3.7 | Empate en puja más baja: gana el que pujó primero en la línea de tiempo | ✅ Cumplido | `contracts/HealthProcurementAuction.sol` — `commitOrder` determina orden; primer commit con precio mínimo es ganador |
| 3.8 | Depósito mínimo 10% de la puja en el momento de pujar | ✅ Cumplido | `contracts/HealthProcurementAuction.sol` — `require(b.deposit * 10 >= _amount)` en fase reveal |
| 3.9 | Al ganador: devolución del depósito + coste de suministro; si no entrega en plazo, pierde depósito | ✅ Cumplido | `contracts/HealthProcurementAuction.sol` — `confirmDeliveryAndPay()` libera pago; `markNonDelivery()` retiene depósito |
| 3.10 | A todos los demás pujadores: devolución del depósito tras finalizar subasta | ✅ Cumplido | `contracts/HealthProcurementAuction.sol` — `withdrawDeposit()` para no-ganadores tras finalizar |
| 3.11 | Pujadores pueden conocer ganador y todas las pujas al finalizar | ✅ Cumplido | `contracts/HealthProcurementAuction.sol` — eventos y estado público tras reveal; `getAllRevealedBids()` expone pujas reveladas |
| 3.12 | Smart contract sin vulnerabilidades de seguridad conocidas | ✅ Cumplido | `contracts/HealthProcurementAuction.sol` — `nonReentrant`, patrón checks-effects-interactions, validación de timestamps; documentado en `Memoria_Consulta_3.md` |
| 3.13 | Reutilizable: inicializable múltiples veces (no sirve solo para una subasta) | ✅ Cumplido | `contracts/HealthProcurementAuction.sol` — patrón Factory (`HealthProcurementAuctionFactory`); cada instancia es contrato independiente |
| 3.14 | Parámetros inicializables: fecha inicio, fecha fin, precio máximo | ✅ Cumplido | `contracts/HealthProcurementAuction.sol` — constructor acepta `auctionStart`, `biddingDeadline`, `revealDeadline`, `deliveryDeadline`, `maxPrice`; verificado en `scripts/deploy.js` |
| 3.15 | Si hay una sola puja válida ≤ maxPrice, el Servicio paga ese valor | ✅ Cumplido | `contracts/HealthProcurementAuction.sol` — caso especial single-bid: precio pagado = bid del único pujador |
...
| 3.17 | Informe detallado de pruebas de mitigación de riesgos y resultados | ✅ Cumplido | `reporte-mythril.md`: 0 vulnerabilidades detectadas. `reporte-slither.json`: 20 detecciones — **0 High/Medium**; `reporte-solhint.txt`: 0 errores y advertencias de estilo/NatSpec/gas. Contrato sin vulnerabilidades de seguridad explotables |
| 3.18 | Proceso de despliegue en Ethereum Testnet (Sepolia) + justificación técnica | ✅ Cumplido | `scripts/deploy.js` + `hardhat.config.js` — flujo completo de despliegue a Sepolia; revisión final ejecutó `npm run deploy:local` correctamente; `Memoria_Consulta_3.md` justifica elección de Sepolia |
...
| 3.21 | Resultados de pruebas dinámicas en Testnet Sepolia (evaluación Blue/Red/Yellow Team) | ⚠️ Parcial | `PlanPruebas.md` incluye matriz de pruebas Sepolia, pero no contiene resultados reales de ejecución en Sepolia (hashes de transacciones, capturas de Etherscan). Requiere wallet de prueba financiada y credenciales RPC/Etherscan; los tests locales de Hardhat pasan con 14 casos |
...
|---|---|
| **1.4** — Firma digital del informe | Firmar el PDF final con DNIe/Autofirma o QSCD y adjuntar el documento firmado como entregable |
